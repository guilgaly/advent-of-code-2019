...
class ComputerTest extends UnitSpec {
    ...
    "given a valid program and position" should {
      "parse it" in {
        ...
        Computer.parseInstruction(program, 0) shouldBe
          Right(Addition(Params2In1Out(Position(9), Position(10), 3)))

        Computer.parseInstruction(program, 4) shouldBe
          Right(Multiplication(Params2In1Out(Position(3), Position(11), 0)))
    ...
    "given a program" should {
      "execute it" in {
        def test(prog: Memory, res: Memory) =
          Computer.executeProgram(prog).map(_.memory) shouldBe Right(res)
